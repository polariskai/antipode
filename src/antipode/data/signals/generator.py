...
class SignalGenerator:
    ...
    def _compute_behavioral_signals(
        ...
        signals['structuring_score'] = self._compute_structuring_score(txns_30d, threshold)
        ...
        # Corridor risk score
        signals['corridor_risk_score'] = self._compute_corridor_risk_score(txns_30d)
    ...
    def _compute_structuring_score(self, transactions: List[Dict], threshold: float) -> int:
        """Count transactions near reporting threshold."""
        margin = threshold * 0.1  # 10% below threshold
        near_threshold = [
            t for t in transactions 
            if threshold - margin < t.get('amount', 0) < threshold
        ]
        return len(near_threshold)
    ...
    def _compute_corridor_risk_score(self, transactions: List[Dict]) -> float:
        """Compute weighted corridor risk score."""
        if not transactions:
            return 0.0

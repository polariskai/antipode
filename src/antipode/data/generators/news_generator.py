    ...
    def _generate_adverse_events(
        ...
        for event_type in selected_types:
            ...
            event = NewsEvent(
                ...
                event_type=event_type,
                severity=[EventSeverity.NEGATIVE, EventSeverity.CRITICAL][int(np.random.random() > 0.7)],
                headline=headline,
                source=self.news_sources['tier1'][np.random.randint(len(self.news_sources['tier1']))] if np.random.random() < 0.5 else self.news_sources['tier2'][np.random.randint(len(self.news_sources['tier2']))],
                source_credibility=SourceCredibility.TIER1 if np.random.random() < 0.5 else SourceCredibility.TIER2,
